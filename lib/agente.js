...
function resolverProvider() {
  ...
  if (pref === 'groq'      && GROQ_KEY)      return 'groq';
  if (pref === 'gemini'    && GEMINI_KEY)    return 'gemini';
  /* auto: primero los gratuitos */
...
async function ejecutarTool(userId, name, input, acciones) {
  ...
  acciones.push({ tool: name, input, ok: !result?.error });
  return result;
}
...
function toolsGemini() {
  return [{
    functionDeclarations: TOOLS.map(t => {
      ...
      if (Object.keys(props).length) fd.parameters = t.input_schema;
      return fd;
    }),
  }];
}

async function chatGemini(userId, msgs) {
  if (typeof fetch !== 'function') throw new Error('fetch no disponible (requiere Node 18+).');
  const acciones = [];
...
async function chat(userId, history) {
  ...
  try {
    if (PROVIDER === 'groq')   return await chatGroq(userId, msgs);
    if (PROVIDER === 'gemini') return await chatGemini(userId, msgs);
    return await chatAnthropic(userId, msgs);
  } catch (e) {
    console.warn(`[agente] error (${PROVIDER}):`, e.message);
    return {

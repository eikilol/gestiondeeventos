...
async function chat(userId, history) {
  ...
  while (guard++ < 6) {
    ...
    /* Caso especial: el modelo pide un formulario estructurado.
       No se ejecuta nada: devolvemos la spec del form a la UI y cortamos.
       El usuario lo llena y responde con los valores (nuevo turno). */
    ...
    if (formBlock) {
      const texto = response.content
        .filter(b => b.type === 'text').map(b => b.text).join('\n').trim();
      const fi = formBlock.input || {};
      return {
        reply: texto || fi.descripcion || fi.titulo || 'Necesito unos datos:',
        mood: 'idle',
        acciones,
        formulario: {
          titulo: fi.titulo || 'Completa estos datos',
          descripcion: fi.descripcion || null,
          campos: Array.isArray(fi.campos) ? fi.campos : [],
        },
      };
    }
